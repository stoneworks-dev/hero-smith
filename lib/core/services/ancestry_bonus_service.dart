...
class AncestryBonusService {
    ...
    if (traits != null) {
      for (final trait in traits) {
        ...
        if (_isPreviousLifeTrait(traitName)) {
          final chosenFormerTraitId = traitChoices[traitId];
          if (chosenFormerTraitId != null && chosenFormerTraitId.isNotEmpty) {
            final formerAncestryId = traitChoices['former_life_ancestry'];
            if (formerAncestryId != null && formerAncestryId.isNotEmpty) {
              final formerTraitBonuses = await _parseFormerLifeTraitBonuses(
                allComponents: allComponents,
                formerAncestryId: formerAncestryId,
                chosenTraitId: chosenFormerTraitId,
                previousLifeTraitId: traitId,
                traitChoices: traitChoices,
              );
              bonuses.addAll(formerTraitBonuses);
            }
          }
        }
      }
  ...
  /// Check if trait name indicates a "Previous Life" trait (Revenant-specific)
  bool _isPreviousLifeTrait(String traitName) {
    return traitName.startsWith('Previous Life:');
  }

  /// Parse bonuses from a trait that was chosen via "Previous Life" (Revenant).
  /// This fetches the trait data from the former ancestry and parses its bonuses.
  Future<List<AncestryBonus>> _parseFormerLifeTraitBonuses({
    required List<db.Component> allComponents,
    required String formerAncestryId,
    required String chosenTraitId,
    required String previousLifeTraitId,
    required Map<String, String> traitChoices,
  }) async {
    // Find the ancestry_trait component for the former ancestry
    final formerTraitsComp = allComponents.firstWhereOrNull((c) {
      if (c.type != 'ancestry_trait') return false;
      try {
        final data = jsonDecode(c.dataJson) as Map<String, dynamic>;
        return data['ancestry_id'] == formerAncestryId;
      } catch (_) {
        return false;
      }
    });
    ...
    // Find the chosen trait in the former ancestry
    for (final trait in traits) {
      if (trait is! Map) continue;
      final traitMap = trait.cast<String, dynamic>();
      final traitId = (traitMap['id'] ?? traitMap['name']).toString();
      ...
      if (immunityChoice != null && immunityChoice.isNotEmpty) {
        modifiedChoices[compositeId] = immunityChoice;
      ...
      if (abilityChoice != null && abilityChoice.isNotEmpty) {
        modifiedChoices[compositeId] = abilityChoice;
        modifiedChoices[chosenTraitId] = abilityChoice;
      }

      // Parse bonuses from the former ancestry trait
      // Use the composite ID to track origin while enabling choice lookups
      return _parseBonusesFromMap(
        traitMap,
        compositeId,
        '$traitName (Previous Life)',
        modifiedChoices,
      );
    }
